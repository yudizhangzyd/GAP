...
#############################################################################
likelihood <- function(response, X, Y = NULL, Z = NULL, size, coeffVec, ncores, hess=TRUE, 
                       weights = NULL, loglikObj = NULL)
{
    ...
    if (is.null(loglikObj)) {
        ...
        probMat <- X %*% matrix(coeffVec[1:((size$K-1) *size$p)],
                                nrow = size$p, ncol = (size$K-1), byrow=FALSE)

        # First compute the utility matrix (stored in probMat: X %*% beta )
        # if (size$p) {
